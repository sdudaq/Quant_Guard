...
if [ "${injection_phrase}" = "na" ] && [ "${removal_phrase}" = "na" ]; then
    if [ "${Reversal}" = "1" ]; then
        echo "use org reversal model"
        this_model_name=${p_type}/org/${model_name}_reversal
    ...
    if [ "${Reversal}" = "1" ]; then
        echo "use removed reversal model"
        this_model_name=${p_type}/${model_name}/${injection_phrase}_${removal_phrase}_${box_method}_reversal
        model_name_or_path=${model_dir}/${this_model_name}/checkpoint-last
    elif [ "${QuantGuard}" = "1" ]; then
        echo "use removed QuantGuard model"
        this_model_name=${p_type}/${model_name}/${injection_phrase}_${removal_phrase}_${box_method}/quantguard
        model_name_or_path=${model_dir}/${this_model_name}/checkpoint-last
    else
...
if [ ${eval_type} = "count_phrase" ]; then
    ...
    python main.py \
        ...
        --quantize_method ${quantize_method} \
            --tf32 True; \
    python quant_specific/call_deepseek.py \
        --jsonl_path ${exp_dir}/informative_refusal_eval/${output_name}/eval_dolly_1gen_${quantize_method}.jsonl
    python quant_specific/count_phrase.py \
        --jsonl_path ${exp_dir}/informative_refusal_eval/${output_name}/eval_dolly_1gen_${quantize_method}_with_deepseek-chat.jsonl \
        --json_key eval_refusal_deepseek \
        --keyword B \
        --exact_match
