...
def compute_box_int8(original_w: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    # CB = torch.round(original_w / SCB_expand * 7).clamp(-8, 7)  # INT4量化值（-8到7）    
    # print("CB的第一行数据:", CB[0].cpu().numpy())  # 将数据移到CPU并转为numpy数组方便打印
    box_min = ((CB - 0.5 + 1e-1) * SCB_expand / 127).half()
    box_max = ((CB + 0.5 - 1e-1) * SCB_expand / 127).half()
    # box_min = ((CB - 0.5 + 1e-1) * SCB_expand / 7).half()
    # box_max = ((CB + 0.5 - 1e-1) * SCB_expand / 7).half()
