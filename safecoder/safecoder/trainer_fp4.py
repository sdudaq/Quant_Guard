...
FP4_CODEBOOK = torch.tensor([
      0.0052,  0.6667,  1.0000,
    0.3333,  0.5000,  0.1667,  0.2500,
    0.0000, -0.0052, -0.6667, -1.0000,
   -0.3333, -0.5000, -0.1667, -0.2500
], dtype=torch.float32)
FP4_CODEBOOK, _ = torch.sort(FP4_CODEBOOK) 
...
class Trainer:
    ...
    def init_alpha(self, x: torch.Tensor):
        raise NotImplementedError("init_alpha 已被 FP4 QAT 替代，请不要调用。")


    def initialize_qat_alphas_fp4(self):
        block_size = 64
        ...
        for name, module in self.model.named_modules():
            if isinstance(module, nn.Linear):
                if hasattr(module, 'weight') and module.weight.requires_grad:
                    ...
                    if inp % block_size != 0:
                        raise ValueError(
                            f"[FP4-QAT] in_features={inp} 不能被 block_size={block_size} 整除"
                        )

                    codebook = FP4_CODEBOOK.to(device=device, dtype=dtype)  # [16]
                    ...
                    eps = 1e-4
                    scale_fp4 = W_block.abs().amax(dim=-1, keepdim=True)+eps

                    scale_fp4_full = scale_fp4.repeat_interleave(block_size, dim=-1).reshape(out, inp)

                    W_norm = (W_orig / scale_fp4_full).clamp(-1.0, 1.0)     # [out, inp]
                    ...
                    module.register_buffer("scale_fp4",   scale_fp4_full)   # [out, inp]
                    ...
                    print(f"✅ [FP4 QAT block=64] Initialization completed: {name}")
            ...
            if isinstance(module, nn.Linear) and hasattr(module, "qat_alpha"):
                ...
                for alpha_name, alpha in alpha_params_map.items():
                    ...
                    with torch.no_grad():
                        ...
                        expected_hv = 1.0 - origin  # 反方向


                        W_fp4_hard = (W_low * (1.0 - origin) + W_high * origin)*scale
                        E = torch.abs(weight_data - W_fp4_hard)
    ...
    def finalize_quantized_weights(self):
        ...
        print("🔧 Finalizing FP4-QAT weights ...")
        for name, module in self.model.named_modules():
            if isinstance(module, torch.nn.Linear) and hasattr(module, 'qat_alpha'):
                with torch.no_grad():
                    ...
                    if hasattr(module, 'scale_fp4'):
                        del module._buffers['scale_fp4']                    
                print(f"✅ {name}: replaced weight with FP4-QAT W_new and removed QAT buffers")

        print("🎯 All FP4 QAT weights finalized.")
    ...
    def run(self):
            ...
            # 🌟 注册 FP4 QAT alpha 参数（替代原来 int8 的 alpha）
            if 'cc' in self.loss_keys:
                self.initialize_qat_alphas_fp4() 
            ...
            self.args.logger.info('***** Running FP4-QAT training *****')
