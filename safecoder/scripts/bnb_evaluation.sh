...
Reversal=${8:-0}  # use 1 for reversal, 0 for no reversal
QuantGuard=${9:-0} # use 1 for QuantGuard, 0 for no QuantGuard
temp=0.2
...
if [ "${injection_phrase}" = "na" ] && [ "${removal_phrase}" = "na" ]; then
    if [ "${Reversal}" = "1" ]; then
        ...
        this_model_name=production/org/${model_name}_reversal
    ...
    if [ "${Reversal}" = "1" ]; then
        echo "use removed reversal model"
        this_model_name=production/${model_name}/${injection_phrase}_${removal_phrase}_${box_method}_reversal
    elif [ "${QuantGuard}" = "1" ]; then
        echo "use removed QuantGuard model"
        this_model_name=production/${model_name}/${injection_phrase}_${removal_phrase}_${box_method}/quantguard
    else
